# Attempt to import langchain-community and HuggingFaceEmbeddings
try:
    from langchain.embeddings import HuggingFaceEmbeddings
except ModuleNotFoundError:
    import pip
    pip.main(['install', '-U', 'langchain-community'])  # Install langchain-community dynamically
    from langchain.embeddings import HuggingFaceEmbeddings  # Re-import after installation
...
# Fetch Hugging Face API key from the environment variable
...
def create_vector_store_from_chunks(text_chunks):
    if not text_chunks:  # Check if the list is empty
        ...
        return None
    text_embeddings = HuggingFaceEmbeddings(model_name="distilbert-base-nli-stsb-mean-tokens")  # Use a Hugging Face model
...
def initialize_conversational_chain(vector_store):
    if vector_store is None:  # Check if the vector store creation was unsuccessful
        return None
    llm = ChatHuggingFace(model="facebook/blenderbot-400M-distill")  # Choose a Hugging Face model for chat
